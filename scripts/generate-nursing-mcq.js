#!/usr/bin/env node
/**
 * Generate NLE Nursing MCQ Questions for PharmRoo
 *
 * สร้างข้อสอบ NLE style ต้นฉบับโดยใช้ Claude API แล้ว import เข้า Supabase
 *
 * Usage:
 *   node scripts/generate-nursing-mcq.js                        # generate ทุก subject
 *   node scripts/generate-nursing-mcq.js --subject NursingAdult # เฉพาะ subject
 *   node scripts/generate-nursing-mcq.js --count 30             # 30 ข้อต่อ subject
 *   node scripts/generate-nursing-mcq.js --dry-run              # generate แต่ไม่ import
 */
...
// ============================================================
// Subject definitions (ตรงกับ mcq_subjects.name ใน DB)
// ============================================================
...
// ============================================================
// Prompt builder (NLE — 4 choices A-D)
// ============================================================

function buildGenerationPrompt(subject, topicAreas, batchSize, batchIndex) {
  const n = topicAreas.length;
  const start = (batchIndex * 3) % n;
  const rotated = [...topicAreas.slice(start), ...topicAreas.slice(0, start)];
  const selectedTopics = rotated.slice(0, Math.min(4, n));
...
async function main() {
  console.log("=".repeat(55));
  console.log("  PharmRoo — Generate NLE Nursing MCQ Questions");
  console.log("=".repeat(55));
  ...
  console.log("[1] Loading nursing subjects from Supabase...");
  ...
  if (subErr || !dbSubjects) {
    console.error("  ERROR: ไม่สามารถดึง nursing subjects จาก Supabase ได้");
    console.error("  " + (subErr?.message || "no data"));
    console.error("  ตรวจสอบ .env.local และว่ารัน create_nursing_subjects.js แล้วหรือยัง");
    process.exit(1);
  }
