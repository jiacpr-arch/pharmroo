...
async function PracticeContent({
  ...
  const selectedQuestions =
    track === "ip1"
      ? [...IP1_PILOT_050, ...IP1_SET2_DAY01, ...IP1_SET2_DAY02, ...IP1_SET2_DAY03, ...IP1_SET2_DAY04]
      : rawQuestions;
        ...
        <div className="mb-8 rounded-2xl border bg-white p-6">
          ...
          {track === "ip1" && (
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">+ Daily Set 2 (ทยอยอัปเดตวันละ 10 ข้อ) · ตอนนี้มี {IP1_SET2_DAY01.length + IP1_SET2_DAY02.length + IP1_SET2_DAY03.length + IP1_SET2_DAY04.length} ข้อ — Day 1: Cleanroom/HVAC/GMP Grade · Day 2: Impurity/Cleaning/Elemental/Scale-up calculations · Day 3: Sterilization/Aseptic processing validation · Day 4: Process capability/Sampling/Qualification/Tech transfer</p>
            </div>
          )}
          {track === "phcp1" && (
            <div>
